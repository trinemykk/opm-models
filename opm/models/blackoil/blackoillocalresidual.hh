                     ...
                     unsigned timeIdx) const
    {
		...
		if(oilVaporizationControl.drsdtConvective()) {
                ...
                || elemCtx.intensiveQuantities(scvf.exteriorIndex(), timeIdx).fluidState().Rs() > 0)
            {
                ...
                const auto rho_in = Opm::getValue(intQuantsIn.fluidState().invB(oilPhaseIdx)) 
                                    * FluidSystem::oilPvt().oilReferenceDensity(intQuantsIn.pvtRegionIndex());; 
                    //paper:::  intQuantsIn.fluidState().density(oilPhaseIdx);
                ...
                const auto rho_ex = Opm::getValue(intQuantsEx.fluidState().invB(oilPhaseIdx))
                                * FluidSystem::oilPvt().oilReferenceDensity(intQuantsEx.pvtRegionIndex());;  
                    //paper::::Opm::getValue(intQuantsEx.fluidState().density(oilPhaseIdx));
